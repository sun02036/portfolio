...
update attachment set status = 'N' where board_no = 81;

-- join: board와 attachment
-- inner join: 기준 컬럼의 값이 null이거나 상대 테이블에서 상응하는 행이 없는 경우 제외
-- left outer join: 기준 테이블 왼쪽의 모든 레코드 출력
-- right outer join: 기준 테이블 오른쪽의 모든 레코드 출력
-- 1:N 관계에서 기준은 항상 1쪽이 되어야 함
